  ...
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      let metadata = JSON.stringify(
        {
          name: `${/[^.]*/.exec(files[i])[0]}`, //names the file based on image (.png) name (ex bitghost10.jpg)
          description: "Bit Ghosts",
      ...
      nftsData.push(
        metadata.slice(0, -2) +
          `,\n\t"img": "${img}"` +
          `,\n\t"id": ${i + 1}\n}` // CHANGE HERE TOO ID (current ghost plus 1) +11
      );
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      nftsData[i] =
        ...
        `,\n\t"price": ${await nft.price(
          i + 1
        )},\n\t"uri": "${await nft.tokenURI(i + 1)}"\n}`; //add previous mint # here to continue +11
      console.log(
        `\n${i + 1} NFT is minted with URI:\n${await nft.tokenURI(i + 1)}` //console log sake +11
      );
    }
